...
def main():
    ...
    file_type = st.sidebar.radio(
        "Type de fichier à analyser", ["Vidéo", "Image"], horizontal=True
    )
    ...
    if file_type == "Vidéo":
        ...
        input_file = st.sidebar.file_uploader(
            "Upload an image file", type=["jpg", "jpeg", "png", "bmp", "tiff"]
        )
    ...
    model_players = YOLO(
        os.path.join(script_dir, "../models/Yolo8L Players/weights/best.pt")
    )
    # Load the YOLOv8 field keypoints detection model
    model_keypoints = YOLO(
        os.path.join(script_dir, "../models/Yolo8M Field Keypoints/weights/best.pt")
    )
    ...
    with tab2:
        if tempf is None:
            st.warning(
                "Veuillez d'abord télécharger une image pour configurer les couleurs des équipes."
            )
        else:
            ...
            with t1col1:
                ...
                with cp1:
                    ...
                    team1_p_color = st.color_picker(
                        label=" ", value=hex_color_1, key="t1p"
                    )
                ...
                with cp3:
                    ...
                    team2_p_color = st.color_picker(
                        label=" ", value=hex_color_3, key="t2p"
                    )
            ...
            with t1col2:
                ...
                if success:
                    extracted_frame.image(
                        frame, use_container_width=True, channels="BGR"
                    )
        ...
        with t2col2:
            ...
            if save_output:
                if file_type == "Vidéo":
                    ...
                    output_file_name = st.text_input(
                        label="File Name (Optional)",
                        placeholder="Enter output image file name.",
                    )
            else:
    ...
    if start_detection and not stop_detection:
        if file_type == "Vidéo":
            ...
            try:
                ...
                if not cap.isOpened():
                    st.error(
                        "Error opening video file. Please check the file path and format."
                    )
                    return
        ...
        if file_type == "Vidéo":
            ...
            if save_output:
                ...
                if os.path.exists(output_path):
                    ...
                    st.warning(
                        "La vidéo a été traitée mais le fichier de sortie n'a pas été trouvé."
                    )
        else:
