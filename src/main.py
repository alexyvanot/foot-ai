...
from detection import create_colors_info, detect, analyze_image
...
def main():
    ...
    if file_type == "Vidéo":
        input_file = st.sidebar.file_uploader('Upload a video file', type=['mp4','mov', 'avi', 'm4v', 'asf'])
    else:
        input_file = st.sidebar.file_uploader('Upload an image file', type=['jpg', 'jpeg', 'png', 'bmp', 'tiff'])

    # Get the directory of the current script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    demo_vid_paths={
        "Demo 1": os.path.join(script_dir, 'demo_vid_1.mp4'),
        "Demo 2": os.path.join(script_dir, 'demo_vid_2.mp4')
    }
    ...
    if file_type == "Vidéo":
        ...
        if not input_file:
            # Image de démo par défaut (on peut extraire une frame des vidéos de démo)
            cap_demo = cv2.VideoCapture(demo_vid_path)
            ret, demo_frame = cap_demo.read()
            cap_demo.release()
            if ret:
                cv2.imwrite(tempf.name, demo_frame)
                st.sidebar.text('Demo image (extracted from demo video)')
                st.sidebar.image(tempf.name)
            else:
                st.sidebar.warning("Erreur lors de l'extraction de l'image de démo")
                tempf = None
        else:
            tempf.write(input_file.read())
    ...
    model_players = YOLO(os.path.join(script_dir, "../models/Yolo8L Players/weights/best.pt"))
    # Load the YOLOv8 field keypoints detection model
    model_keypoints = YOLO(os.path.join(script_dir, "../models/Yolo8M Field Keypoints/weights/best.pt"))
    ...
    with tab1:
        ...
        st.markdown("""
                    ...
                    **For Videos:**
                    1. Select "Vidéo" in the Media Upload section and upload a video file, or use the demo videos.
                    ...
                    6. Go to the "Model Hyperparameters & Detection" tab, adjust hyperparameters and select the annotation options.
                    7. Run Detection!
                    8. If "save outputs" option was selected the saved video can be found in the "outputs" directory.
                    
                    **For Images:**
                    1. Select "Image" in the Media Upload section and upload an image file (JPG, PNG, etc.).
                    2. Enter the team names that correspond to the uploaded image.
                    3. Access the "Team colors" tab to pick team colors from detected players.
                    4. Go to the "Model Hyperparameters & Detection" tab and run analysis.
                    5. If "save outputs" option was selected the analyzed image will be saved in the "outputs" directory.
                    """)
        st.write("Version 0.0.3")

    with tab2:
        if tempf is None:
            st.warning("Veuillez d'abord télécharger une image pour configurer les couleurs des équipes.")
        else:
            ...
            with t1col1:
                ...
                if success:
                    with st.spinner('Detecting players in selected frame/image...'):
                        ...
                        if len(detections_imgs_list) > 0:
                            ...
                            st.warning("Aucun joueur détecté dans cette frame/image.")
                            concat_det_imgs = None
                            
                    st.write("Detected players")
                    if concat_det_imgs is not None:
                ...
                if value is not None and concat_det_imgs is not None:
    ...
    with tab3:
        ...
        with t2col2:
            ...
            if save_output:
                if file_type == "Vidéo":
                    ...
                    output_file_name = st.text_input(label='File Name (Optional)', placeholder='Enter output image file name.')
            else:
        ...
        if file_type == "Vidéo":
            ...
            # Pour les images, pas besoin de paramètres de tracking
            ball_track_hyperparams = {
                0: 30,
                1: 100,
                2: 35
            ...
            with bcol22t:
                show_pal = st.toggle(label="Show Color Palettes", value=True)
                show_b = st.toggle(label="Show Ball Position", value=True)
            plot_hyperparams = {
                0: show_k,
                1: show_pal,
                2: show_b,
        ...
        with bcol22:
            ...
            if file_type == "Vidéo":
                ...
                start_detection = st.button(label='Analyze Image', disabled=ready)
        with bcol23:
            if file_type == "Vidéo":
                ...
                stop_detection = False  # Pas de bouton stop pour les images
        with bcol24:
            st.write('')
    ...
    if start_detection and not stop_detection:
        if file_type == "Vidéo":
            st.toast(f'Starting video detection...')
            try:
            ...
            st.toast(f'Starting image analysis...')
            try:
                if tempf is None:
                    st.error("No image file available for analysis.")
                    return
                    
                status = analyze_image(tempf.name, stframe, output_file_name, save_output, model_players, model_keypoints,
                                       detection_hyper_params, plot_hyperparams, num_pal_colors, colors_dic, color_list_lab)
            except Exception as e:
    ...
    if status:
        if file_type == "Vidéo":
            st.success('Video detection finished successfully!')
            if save_output:
            ...
            st.success('Image analysis finished successfully!')
