...
def get_labels_dics():
    ...
    json_path = os.path.join(script_dir, "../pitch map labels position.json")
    with open(json_path, "r") as f:
        keypoints_map_pos = json.load(f)

    # Get football field keypoints numerical to alphabetical mapping
    yaml_path = os.path.join(script_dir, "../config pitch dataset.yaml")
    with open(yaml_path, "r") as file:
        classes_names_dic = yaml.safe_load(file)
    classes_names_dic = classes_names_dic["names"]

    # Get football field keypoints numerical to alphabetical mapping
    yaml_path = os.path.join(script_dir, "../config players dataset.yaml")
    with open(yaml_path, "r") as file:
...
def generate_file_name():
    ...
    list_video_files = os.listdir(outputs_dir)
    idx = 0
    while True:
        idx += 1
...
def detect(
    ...
    if save_output:
        ...
        output_path = f"./outputs/{output_file_name}.mp4"
        # Use a more compatible codec
        fourcc = cv2.VideoWriter_fourcc(*"mp4v")
        output = cv2.VideoWriter(output_path, fourcc, 30.0, (width, height))

        if not output.isOpened():
            st.error(f"Erreur : Impossible de créer le fichier vidéo {output_path}")
            return False
    ...
    try:
        for frame_nbr in range(1, tot_nbr_frames + 1):
            ...
            st_prog_bar.progress(
                percent_complete, text=f"Détection en cours ({percent_complete}%)"
            )
            ...
            if success:
                ...
                if "homog" in locals():
                    ...
                    for (
                        pt
                    ) in detected_ppos_src_pts:  # Loop over players frame coordiantes
                        pt = np.append(
                ...
                # Display the annotated frame (every 5th frame to reduce memory usage)
                if frame_nbr % 5 == 0:  # Display every 5th frame
                    try:
                        # Convert to RGB for better Streamlit compatibility
                        final_img_rgb = cv2.cvtColor(final_img, cv2.COLOR_BGR2RGB)
                        stframe.image(final_img_rgb, channels="RGB")
                    except Exception as e:
                        print(f"Warning: Could not display frame {frame_nbr}: {e}")
                ...
                if save_output:
                    try:
                        ...
                        print(f"Error writing frame {frame_nbr}: {e}")

            else:
                # If frame reading failed, break the loop
                print(f"Warning: Could not read frame {frame_nbr}")
                break
    ...
    # Clean up resources
    try:
        if save_output and "output" in locals():
            output.release()
            print(f"Video saved successfully to ./outputs/{output_file_name}.mp4")
    except Exception as e:
...
def analyze_image(
    ...
    with st.spinner("Analyzing image..."):
        ...
        if len(detected_labels) > 3:
            homog, mask = cv2.findHomography(
                detected_labels_src_pts, detected_labels_dst_pts
            )
        ...
        annotated_frame = cv2.copyMakeBorder(
            annotated_frame, 40, 10, 10, 10, cv2.BORDER_CONSTANT, value=border_color
        )
        tac_map_copy = cv2.copyMakeBorder(
            tac_map_copy, 70, 50, 10, 10, cv2.BORDER_CONSTANT, value=border_color
        )
        tac_map_copy = cv2.resize(
            tac_map_copy, (tac_map_copy.shape[1], annotated_frame.shape[0])
        )
        ...
        ## Add info annotation
        cv2.putText(
            final_img, "2D Map", (1370, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 0), 2
        )
        cv2.putText(
            final_img,
            "Image Analysis",
